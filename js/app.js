...
function addUserItem(text) {
  const appContent = document.querySelector(".app-content");
  appContent.innerHTML += '<div class="item-container item-container-user"><div class="item"><p class="userItems">' + text + '</p></div></div>';
  appContent.scrollTop = appContent.scrollHeight; // scroll to bottom
}
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (!isChrome()) {
    ...
    addError("Your browser doesn’t support speech synthesis. This demo won’t work,Chat with me");
    return;
  }
  ...
  // api.ai client
  const apiClient = new ApiAi.ApiAiClient({accessToken: '76afa13332aa45a99cee439f4702f0c8'});

  // Initial feedback message.
  var speech="Hi! I’m PIK. Tap the microphone and start talking to me.";
  addBotItem(speech);
